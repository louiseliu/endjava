...
function EndSkin(tmpId)
{
	...
	this.compile = function(page)
	{
		...
		//remove remark,two kinds of remarks
		if(ms = page.match(/\<\%\-\-.*\-\-\%\>/ig)){
			for(i=0; m=ms[i]; i++)
			{
				page = page.replace(m, '');
		...
		if (ms = page.match(/\<c\:set.*\>/ig))
		{
			for(i=0; m=ms[i]; i++)
			{
				var _ms = m.match(/\<c\:set.*?var=\"(.*?)\".*?value=\"(\$\{.*?\}).*?\"\>/i);//if value of cset is variable like 'value="${num}"'
				if (_ms)
				{
					var value = _ms[2];
					var variable = '${'+_ms[1]+'}';
...
//		console.log("codes:" + codes.join(''));
		...
		{
			var err = [];
			if(cSetBegins > cSetEnds) err.push('missing ' + (cSetBegins-cSetEnds) + '{/set}');
			if(cSetBegins < cSetEnds) err.push('too much {/set}');
